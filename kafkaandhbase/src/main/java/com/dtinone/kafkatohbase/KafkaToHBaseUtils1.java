...
import com.dtinone.earlywarningrules.Warning1;
...
public class KafkaToHBaseUtils1 {
  ...
  */
        ...
        //调用方法获取预警等级
        final int preLevel1 = new Warning1().PressureWarning(Integer.parseInt(pressure));
        final String preLevel = String.valueOf(preLevel1);
        final int temLevel1 = new Warning1().TemperatureWarning(Integer.parseInt(temperature));
        final String temLevel = String.valueOf(preLevel1);
        final int floLevel1 = new Warning1().FlowWarning(Integer.parseInt(flow));
        final String floLevel = String.valueOf(floLevel1);

        Random random = new Random();
        final int state1 = random.nextInt(3)-1; //-1:默认状态，0：确认告警，1：误报(暂时随机生成)
        final String state = String.valueOf(state1);
        //System.out.println(i);

        //写入HBbase的字段：id,type,addr,pressure(pa),
        // temperature(℃),flow(m³/s),time,area,preLevel,temLevel,floLevel,state
        init(); //连接
        String colF[] ={"info"};
        ...
        insertData("detail",rowkey0,"info","preLevel",preLevel);
        insertData("detail",rowkey0,"info","temLevel",temLevel);
        insertData("detail",rowkey0,"info","floLevel",floLevel);
        insertData("detail",rowkey0,"info","state",state);
